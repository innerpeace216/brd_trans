...
enum TypeShape{Circular=0, Polygon=1, Na=-1};//修改值；
...
private:
	...
	// "Blind": drill hole does not go through all layers, but includes either Top or Bottom layer
	...
	std::vector<std::vector<Point2D>> pad_shapes_;//问题；
	...
	// if PadShape is circular, then PadData(1) is center; (2) is on the circle.改动：(若为circular，data1/2都是直径）
...
private:
	...
	//string Net; // net name to which this via belongs to
	enum TypeVia type_; // PTH, Blind, Buried, Drilled;问题；
	int net_index_; // net name index to which this via belongs to；与net_list关联；
	int color_;
	Point2D location_; 
	int padstack_index_;
	std::vector<int> pad_exist_; 
	double rotation_;//需添加cmd命令；
	int back_drill_layer_start_;  // starting from this layer, the via is drilled out (removed)
	int back_drill_layer_stop_;   // the backdrill (i.e. removal of via) is stopped at this layer
	// the start layer number is always smaller than the stop layer number
	DblMNM back_drill_diameter_;//需添加命令；
};
...
private:
	...
	int layer_; // top or bottom placement；问题（fcomp和fsymbol都没有）；
	...
	std::vector<int> net_indices_; // 问题(同一个refdes的pin，可能有不同的net；因此comp对象是以pin为单位还是refdes？）；list of nets connected to the pins, in the same sequence as the PinNames in Symbol
...
private:
	...
	std::vector<std::vector<Point2D>> pin_shapes_;//问题；
